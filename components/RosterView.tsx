...
import { Shield, TrendingUp, Battery, AlertCircle, Briefcase, Search, Filter, UserMinus, DollarSign, RefreshCw, CheckCircle2, Loader2, GripVertical, ChevronUp, ChevronDown, Activity, HeartPulse, ShieldAlert, FileText, X, Info, Calendar, Award, Sparkles, Heart, Zap } from 'lucide-react';
import ContractNegotiation from './ContractNegotiation';
import { RestructureModal, ReleasePlayerModal } from './CapModals';
import DepthChartVisualizer from './DepthChartVisualizer';
...
const RosterView: React.FC<RosterViewProps> = ({ selectedTeamId, allPlayers, setAllPlayers, teams }) => {
  ...
  const [viewMode, setViewMode] = useState<'standard' | 'progression' | 'financial' | 'depth_chart'>('standard');
  ...
  const activeProgressionPlayer = allPlayers.find(p => p.id === viewingProgressionPlayerId);
  ...
  const handleAutoGenerateDepthChart = () => {
    ...
    allPositions.forEach(pos => {
      const posPlayers = allPlayers.filter(p => p.teamId === selectedTeamId && p.position === pos);
      // Sort strictly by highest overall / scheme overall first to fill starting positions efficiently
    ...
    const updatedPlayers = allPlayers.map(p => {
      if (p.teamId === selectedTeamId && depthMap.has(p.id)) {
        return {
          ...p,
  ...
  return (
    <div className="p-8 h-full overflow-hidden flex flex-col bg-[#05070a]">
      <header className="mb-10 flex justify-between items-end border-b border-[#1a222e] pb-6">
        ...
        <div className="flex gap-4 items-center">
          <button
            ...
            className={`flex items-center gap-2.5 px-5 py-3 border transition-all font-bold uppercase tracking-[0.2em] text-[10px] mono-font ${
              autoGenSuccess
          ...
          >
            {autoGenSuccess ? <CheckCircle2 size={16} /> : <Sparkles size={16} />}
            {autoGenSuccess ? 'DEPTH_CHART_OPTIMIZED' : 'AUTO_GENERATE_DEPTH_CHART'}
          </button>
      ...
      <div className="bg-[#0a0e14] border border-[#1a222e] flex-1 flex flex-col overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.5)]">
          ...
          <div className="flex gap-2 items-center">
            ...
            <div className="flex bg-[#05070a] p-1 border border-[#1a222e]">
              <button 
                ...
                className={`px-4 py-1.5 text-[9px] font-bold tracking-[0.2em] uppercase transition-all ${viewMode === 'standard' ? 'bg-cyan-500 text-black' : 'text-slate-600 hover:text-slate-300'}`}
              >
                TACTICAL
              </button>
              <button 
                onClick={() => setViewMode('progression')}
                className={`px-4 py-1.5 text-[9px] font-bold tracking-[0.2em] uppercase transition-all flex items-center gap-1.5 ${viewMode === 'progression' ? 'bg-cyan-500 text-black' : 'text-slate-600 hover:text-cyan-400'}`}
              >
                <Sparkles size={11} className={viewMode === 'progression' ? 'text-black' : 'text-cyan-400'} />
                DEV & MORALE
              </button>
              <button 
                onClick={() => setViewMode('financial')}
                className={`px-4 py-1.5 text-[9px] font-bold tracking-[0.2em] uppercase transition-all ${viewMode === 'financial' ? 'bg-cyan-500 text-black' : 'text-slate-600 hover:text-slate-300'}`}
              >
                FINANCIAL
              </button>
              <button 
                onClick={() => setViewMode('depth_chart')}
                className={`px-4 py-1.5 text-[9px] font-bold tracking-[0.2em] uppercase transition-all ${viewMode === 'depth_chart' ? 'bg-cyan-500 text-black' : 'text-slate-600 hover:text-slate-300'}`}
              >
                DEPTH CHART
              </button>
            </div>
        ...
        {viewMode === 'depth_chart' ? (
          <div className="flex-1 overflow-y-auto p-6 space-y-8 bg-[#05070a]/50">
            <DepthChartVisualizer
              teamId={selectedTeamId}
              team={team}
              allPlayers={allPlayers}
              setAllPlayers={setAllPlayers}
              onExtendPlayer={(p) => setNegotiatingPlayerId(p.id)}
              onRestructurePlayer={(p) => setRestructuringPlayerId(p.id)}
            />
          </div>
        ) : (
          <>
                ...
                <thead className="sticky top-0 bg-[#0d121a] z-10">
                  <tr className="text-[10px] text-slate-500 uppercase tracking-[0.3em] font-bold border-b border-[#1a222e] mono-font">
                    ...
                    {viewMode === 'standard' ? (
                      ...
                      <>
                        <th className="px-4 py-5 font-bold text-center">OVR / DEV</th>
                        <th className="px-4 py-5 font-bold">PROGRESSION_XP</th>
                        <th className="px-4 py-5 font-bold text-center">SKILL_POINTS</th>
                        <th className="px-4 py-5 font-bold text-center">MORALE</th>
                      </>
                    ) : (
                      <>
                      ...
                      <tr key={player.id} className="hover:bg-cyan-500/5 transition-all duration-300 group">
                        <td className="px-6 py-4">
                          <div className="flex items-center gap-4">
                            ...
                            <div>
                              <div 
                                onClick={() => setViewingProgressionPlayerId(player.id)}
                                className="text-sm font-bold text-white group-hover:text-cyan-400 cursor-pointer transition-colors uppercase tracking-tight italic header-font flex items-center gap-2"
                              >
                                {player.name}
                                {skillPoints > 0 && (
                          ...
                          <>
                            <td className="px-4 py-4 text-center">
                              <div className="flex flex-col items-center">
                                <span className="text-base font-mono font-bold text-white leading-none">{player.overall}</span>
                                <span className={`text-[8px] font-mono font-bold mt-1 px-1.5 py-0.2 border ${
                                  player.developmentTrait === 'X-Factor' ? 'text-amber-400 border-amber-500/40 bg-amber-500/10' :
                                  player.developmentTrait === 'Superstar' ? 'text-cyan-400 border-cyan-500/40 bg-cyan-500/10' :
                                  player.developmentTrait === 'Star' ? 'text-emerald-400 border-emerald-500/40 bg-emerald-500/10' :
                            ...
                            <td className="px-4 py-4 min-w-[160px]">
                              <div className="space-y-1">
                                ...
                                <div className="h-1.5 w-full bg-slate-900 border border-slate-800 overflow-hidden">
                                  <div 
                                    className="h-full bg-cyan-400 shadow-[0_0_8px_rgba(0,209,255,0.6)]" 
                                    style={{ width: `${xpPct}%` }}
                                  />
                                </div>
                              </div>
                            ...
                            <td className="px-4 py-4 text-center">
                              {skillPoints > 0 ? (
                                <button
                                  onClick={() => setViewingProgressionPlayerId(player.id)}
                                  className="px-2.5 py-1 bg-cyan-500 text-black font-mono font-bold text-[10px] animate-pulse shadow-[0_0_10px_rgba(0,209,255,0.4)] flex items-center gap-1 mx-auto"
                                >
                                  <Sparkles size={11} />
                                  <span>{skillPoints} READY</span>
                                </button>
                              ) : (
                            ...
                            <td className="px-4 py-4 text-center">
                              <span className={`px-2.5 py-1 text-[10px] font-mono font-bold border ${
                                player.morale >= 85 ? 'text-emerald-400 bg-emerald-500/10 border-emerald-500/30' :
                                player.morale >= 65 ? 'text-cyan-400 bg-cyan-500/10 border-cyan-500/30' :
                                player.morale >= 45 ? 'text-amber-400 bg-amber-500/10 border-amber-500/30' :
                        ...
                        <td className="px-6 py-4 text-right">
                          <div className="flex justify-end gap-1 opacity-0 group-hover:opacity-100 transition-all translate-x-2 group-hover:translate-x-0">
                            <button
                              ...
                              title="Development & Progression Matrix"
                            >
                              <Sparkles size={12} />
                              DEV
                            </button>
      ...
      {activeProgressionPlayer && (
        <PlayerProgressionModal
          player={activeProgressionPlayer}
